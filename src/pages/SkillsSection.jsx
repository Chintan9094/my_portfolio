...
const skills = [
  ...
  { name: "Node.js", color: "from-green-500 to-lime-600", glow: "rgba(34, 197, 94, 0.45)" },
  { name: "Express.js", color: "from-gray-400 to-gray-600", glow: "rgba(156, 163, 175, 0.4)" },
  { name: "Socket.io", color: "from-cyan-400 to-teal-500", glow: "rgba(34, 211, 238, 0.4)" },
  { name: "MongoDB", color: "from-emerald-400 to-green-600", glow: "rgba(16, 185, 129, 0.45)" },
    ...
    <section ref={ref} id="skills" className={`relative py-32 px-6 overflow-hidden ${isDarkMode ? "bg-black" : "bg-transparent"}`}>
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-20">
          <h2 className="mb-4 bg-linear-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent">Tech Arsenal</h2>
          ...
          {skills.map((skill, index) => (
            <motion.div key={skill.name} initial={{ opacity: 0, scale: 0 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay: index * 0.1, type: "spring", stiffness: 200 }} whileHover={{ scale: 1.15 }}>
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 3 + (index % 3), repeat: Infinity, ease: "easeInOut" }} className="relative">
                ...
                <div className={`relative w-32 h-32 rounded-full bg-linear-to-br ${skill.color} p-1`}>
                ...
                <motion.div className={`absolute inset-0 rounded-full bg-linear-to-br ${skill.color} opacity-0 group-hover:opacity-20`} animate={{ scale: [1, 1.2, 1], opacity: [0, 0.3, 0] }} transition={{ duration: 2, repeat: Infinity }} />
              </motion.div>
            </motion.div>
          ))}
        </div>
