...
export default function Preloader({ theme = "dark", onFinish }) {
  ...
  const textGradient =
    theme === "dark"
      ? "from-cyan-400 via-green-400 to-purple-500"
    ...
    <div className={`${bgColor} fixed inset-0 flex flex-col items-center justify-center z-50 overflow-hidden`}>
      ...
      >
        {[...Array(3)].map((_, i) => (
          <motion.div
            key={i}
            className={`w-4 h-4 rounded-full ${dotGradient} shadow-[0_0_12px_rgba(0,255,255,0.6)]`}
      ...
      {/* Text */}
      <motion.p
        className={`mt-8 text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r ${textGradient}`}
